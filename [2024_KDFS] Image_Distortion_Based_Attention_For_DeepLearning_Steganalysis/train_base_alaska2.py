    ...
    if args.use_wandb:
        if args.run_name == 'auto':
            if args.stride is not None or args.dropout_rate > 0:
                args.run_name = f'{args.backbone}_stride_{args.stride}_dropout_{args.dropout_rate}_ALASKA2_QF_{args.qf}_{args.batch_size}_{args.lr}'
            else:
                args.run_name = f'{args.backbone}_ALASKA2_{args.stego_method}_QF_{args.qf}_{args.batch_size}_{args.lr}'
            if args.suffix != '':
...
if __name__ == '__main__':
    ...
    # Model Parsers
    parser.add_argument('--backbone', type=str, default='timm-mobilevit_s', help='Backbone name from timm library')
    ...
    parser.add_argument('--stego_method', type=str, default='UERD')
    ...
    if args.use_wandb:
        ...
        wandb.save(f'./train_base_alaska2.py', policy='now')
