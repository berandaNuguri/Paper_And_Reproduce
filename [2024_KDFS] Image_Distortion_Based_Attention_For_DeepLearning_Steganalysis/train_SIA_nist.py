...
def main(args):
    ...
    if args.use_wandb:
        if args.run_name == 'auto':
            args.run_name = f'MobileViT_NIST_{args.stego_method}_bpp_{args.min_embedding_rate}_{args.max_embedding_rate}_w_{args.batch_size}_{args.lr}'
            if args.apply_sia:
                args.run_name += f'_SIA_BeforeViTBlock_{args.attention_type}_{args.rho_scaler}'
            if args.suffix != '':
    ...
    wandb.save('./utils/datasets.py', policy='end')
    wandb.save('./train_SIA_nist.py', policy='end')
    wandb.finish()
...
if __name__ == '__main__':
    ...
    if args.use_wandb:
        wandb.init(project='2024_SCI', entity='kumdingso', save_code=True)
