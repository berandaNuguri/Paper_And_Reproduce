...
from arch.architecture import DSDBNet
from utils.transform_configs import get_transforms, get_transforms_albu
...
def main(args, device):
    if args.use_wandb:
        if args.run_name != '':
            ...
            wandb.run.name = f'2024_KCC_protocol2'
        wandb.save(f'./utils/datasets.py')
        wandb.save(f'./utils/transform_configs.py')
        wandb.save(f'./train_2.py')
    ...
    test_path  = f'{args.data_root}/test.txt'

    train_df = pd.read_csv(train_path, header=None, sep=" ", names=['path', 'label'])
    valid_df = pd.read_csv(valid_path, header=None, sep=" ", names=['path', 'label'])
    test_df  = pd.read_csv(test_path, header=None, sep=" ", names=['path', 'label'])
    ...
    test_dataset = SSIDataset(df=test_df, root_path=args.data_root+'/test/', mix_chans=args.mix_chans, transform_1=valid_transform, transform_2=valid_transform)
    ...
    for epoch in range(args.epochs):
        ...
        positive_preds = train_preds[:, 1]

        train_auc = roc_auc_score(train_labels, positive_preds)
        _, _, thresholds = roc_curve(train_labels, [i for i in positive_preds])
        
        optimal_err_cand = []
        optimal_apcer = []
        optimal_bpcer = []
        for thresh in thresholds:
            _conf_mat = confusion_matrix(train_labels, [1 if i >= thresh else 0 for i in positive_preds])
            _tn, _fp, _fn, _tp = _conf_mat.ravel()
            _train_apcer = _fp / (_fp + _tn)
            _train_npcer = _fn / (_fn + _tp)
            _train_acer = (_train_apcer + _train_npcer) / 2
            optimal_err_cand.append(_train_acer)
            optimal_apcer.append(_train_apcer)
            optimal_bpcer.append(_train_npcer)
            
        optimal_idx = np.argmin(optimal_err_cand)
        optimal_train_acer = optimal_err_cand[optimal_idx]
        optimal_train_apcer = optimal_apcer[optimal_idx]
        optimal_train_bpcer = optimal_bpcer[optimal_idx]

        # print(f'Epoch [{epoch}/{args.epochs-1}], Train - Loss: {train_loss[-1]:.4f}, AUC: {train_auc:.4f}, ACER: {optimal_train_acer:.4f}, APCER: {optimal_train_apcer:.4f}, BPCER: {optimal_train_bpcer:.4f}')
...
if __name__ == '__main__':
    ...
    # Model Settings
    parser.add_argument('--arch', type=str, default='MatBackbone', choices=['MatBackbone', 'PngBackbone', 'DSDBNet'])
    ...
    parser.add_argument('--ckpt_root', type=str, default='./ckpt/train_to_test/')
    ...
    if args.use_wandb:
        wandb.init(project='2024_KCC_protocol2', entity='kumdingso')
