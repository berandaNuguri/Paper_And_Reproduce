...
with open(f'./test/train_txt/{dataset_name}/{dataset_name}_probe_train.txt', 'w', encoding='UTF-8') as probe_text:
    for idx, probe_path in enumerate(probe['path']):
        probe_text.write(probe_path+'\t'+ str(probeset['label'][idx]) + '\t'+str(probeset['id'][idx]+'_'+probeset['finger'][idx]) + '\n')

with open(f'./test/train_txt/{dataset_name}/{dataset_name}_template_train.txt', 'w', encoding='UTF-8') as template_text:
    for template_path in template['path']:
        template_text.write(template_path + '\t' + str(template_path.split('/')[-3] + '_' + template_path.split('/')[-1].split(' ')[0]) + '\n')
